...
Custom DAX written for this project, beyond what the course starter file provided:

**Competitor Churn %** (measure) — % of churned customers whose churn category is "Competitor"
```DAX
Competitor Churn % =
DIVIDE(
    CALCULATE(COUNTROWS('Databel - Data'), 'Databel - Data'[Churn Category] = "Competitor"),
    CALCULATE(COUNTROWS('Databel - Data'), 'Databel - Data'[Churn Label] = "Yes")
)
```

**Age Segment** (calculated column) — merges the overlapping Under 30 / Senior flags into one clean 3-category field
```DAX
Age Segment =
SWITCH(
    TRUE(),
    'Databel - Data'[Senior] = "Yes", "Senior (65+)",
    'Databel - Data'[Under 30] = "Yes", "Under 30",
    "30–64"
)
```

**Region Label** (calculated column) — isolates California for the outlier comparison on the Insights page
```DAX
Region Label =
IF('Databel - Data'[State] = "CA", "California", "All Other States")
```

*Note: base measures inherited from the course starter file (e.g. `Churn Rate`) aren't reproduced
here verbatim; the 3 above are the ones written specifically for this project's analysis.*
